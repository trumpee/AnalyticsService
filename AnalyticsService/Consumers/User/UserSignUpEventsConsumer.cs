...
public class UserSignUpEventsConsumer(
    IAnalyticsEventsRepository repository,
    ILogger<UserSignUpEventsConsumer> logger) : IConsumer<Trumpee.MassTransit.Messages.Analytics.AnalyticsEvent<UserSignUpPayload>>
{
    public async Task Consume(ConsumeContext<Trumpee.MassTransit.Messages.Analytics.AnalyticsEvent<UserSignUpPayload>> context)
    {
        try
        {
            var e = context.Message;
            var payload = context.Message.Payload!;
            logger.LogDebug($"User with email {payload.Email} signed up");
